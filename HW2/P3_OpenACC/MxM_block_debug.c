...
int main() {
    ...
    int p,q,r;
    #pragma acc data copyin(N,block_num,A[:N][:N],B[:N][:N], \
                c[:BSIZE][:BSIZE],a[:BSIZE][:BSIZE],b[:BSIZE][:BSIZE]) copy(C[:N][:N]) 
    {

    // p, q, r  are automatically set private. 
    // Unlike OpenMP, here you can't set private before declaring a variable.
    #pragma acc parallel private(c[:BSIZE][:BSIZE],a[:BSIZE][:BSIZE],b[:BSIZE][:BSIZE]) num_gangs(4) vector_length(16) 
    {
        ...
        for (j=0; j<block_num; j++){
            ...
            for(k=0; k<block_num; k++){
                ...
                for (p=0; p<BSIZE; p++){
                for (q=0; q<BSIZE; q++){
                    //a[p][q] = 0.0; //A[i*BSIZE+p][k*BSIZE+q];
                    //b[p][q] = 0.0; //B[k*BSIZE+p][j*BSIZE+q];
                }
                }
            /*  
